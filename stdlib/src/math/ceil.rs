...
pub(crate) const FUNCTION: &str = "ceil";
...
pub(crate) fn callback(context: &mut NativeContext<'_>) -> VmResult<NativeAction> {
    ...
    if value.is_integer() {
        ...
        let ceiled = float.ceil();

        const LIMIT: f64 = 9_223_372_036_854_775_808.0;

        if ceiled.is_finite() && (-LIMIT..LIMIT).contains(&ceiled) {
            Ok(context.return_values([context.integer(ceiled as i64)]))
        } else {
            Ok(context.return_values([context.float(ceiled)]))
        }
    }
}
